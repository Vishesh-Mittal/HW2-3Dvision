...
def draw_correspondences(img, ptsTrue, ptsReproj, ax, drawOnly=50):
    ...
    Returns: 
    - ax: matplotlib axis object with drawings.
    """
    n_pts = ptsTrue.shape[0]
    indices = np.arange(n_pts)
    if drawOnly < n_pts:
        indices = np.random.choice(indices, size=drawOnly, replace=False)
    
    for i in indices:
        true_point = ptsTrue[i]
        reproj_point = ptsReproj[i]

        # Draw line between true and reprojected point
        ax.plot([true_point[0], reproj_point[0]], [true_point[1], reproj_point[1]], 'r-', lw=0.5)
        
        # Mark the true point location
        ax.plot(true_point[0], true_point[1], 'go', markersize=5, label='Ground Truth' if i == indices[0] else "")
        
        # Mark the reprojected point location
        ax.plot(reproj_point[0], reproj_point[1], 'bx', markersize=5, label='Reprojected' if i == indices[0] else "")
    ...
    if handles:
        ax.legend(handles[:2], labels[:2], loc='upper right', fontsize='small')
