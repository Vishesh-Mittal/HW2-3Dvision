...
class SFM(object): 
    ...
    def update_cloud_and_refs(self, prev_name, name, points3D, img1idx, img2idx):
        """
        Updates the 3D point cloud with new points and updates references in the image data.

        Args:
            prev_name (str): Name of the previous image.
            name (str): Name of the current image.
            points3D (numpy.ndarray): Array of new 3D points to add.
            img1idx (numpy.ndarray): Indices of keypoints in the previous image that correspond to the new 3D points.
            img2idx (numpy.ndarray): Indices of keypoints in the current image that correspond to the new 3D points.
        """
        #print("prev_name, name, points3D, img1idx, img2idx=",prev_name, name, points3D.shape, len(img1idx), len(img2idx))
        # Add the new points to the point cloud
        start_idx = self.point_cloud.shape[0]  
        self.point_cloud = np.vstack([self.point_cloud, points3D])

        # Update references in the image data for the previous image
        if prev_name in self.image_data:
            ref1 = self.image_data[prev_name][-1]  # Get the current references
            updated_ref1 = np.copy(ref1)  # Create a copy to avoid modifying the original array during iteration
            for i, idx in enumerate(img1idx):
                if isinstance(idx, np.ndarray):
                    idx = idx.item() 
                updated_ref1[int(idx)] = start_idx + i
            self.image_data[prev_name][-1] = updated_ref1  # Update the image data with modified references

        # Update references in the image data for the current image
        if name in self.image_data:
            ref2 = self.image_data[name][-1]  # Get the current references
            updated_ref2 = np.copy(ref2)  # Create a copy to avoid modifying the original array during iteration
            for i, idx in enumerate(img2idx):
                if isinstance(idx, np.ndarray):
                    idx = idx.item()  
                updated_ref2[int(idx)] = start_idx + i
            self.image_data[name][-1] = updated_ref2  # Update the image data with modified references

    def trangulate_new_view(self, name): 
        """
        Triangulates a new view based on matches with previous views, incorporating outlier removal.
        ...
        for prev_name in self.image_data.keys(): 
            if prev_name != name: 
                ...
                if len(matches) > 0:
                    # Extract locations of matched keypoints for RANSAC
                    pts1 = np.float32([kp1[m.queryIdx].pt for m in matches])
                    pts2 = np.float32([kp2[m.trainIdx].pt for m in matches])

                    # Estimate the Fundamental matrix using RANSAC
                    F, mask = cv2.findFundamentalMat(pts1, pts2, cv2.FM_RANSAC)
                    # Select inlier matches based on the mask returned by RANSAC
                    inlier_matches = [matches[i] for i in range(len(matches)) if mask[i,0]]

                    if len(inlier_matches) > 0:
                        img1pts, img2pts = pts1[mask.ravel() == 1], pts2[mask.ravel() == 1]

                        # Assuming camera matrices (R, t) for both views are already computed
                        R1, t1, _ = self.image_data[prev_name]
                        R_new, t_new, _ = self.image_data[name]

                        # Compute the projection matrices for both views
                        P1 = self.K.dot(np.hstack((R1, t1)))
                        P_new = self.K.dot(np.hstack((R_new, t_new)))

                        # Triangulate points using inliers
                        points4D = cv2.triangulatePoints(P1, P_new, img1pts.T, img2pts.T)
                        points3D = cv2.convertPointsFromHomogeneous(points4D.T).reshape(-1, 3)

                        inlier_indices = np.where(mask.ravel() == 1)[0]
                        img1idx = [matches[i].queryIdx for i in inlier_indices]  # Indices in kp1
                        img2idx = [matches[i].trainIdx for i in inlier_indices]  # Indices in kp2

                        # Update the 3D point cloud and references for both the new and previous views
                        self.update_cloud_and_refs(prev_name, name, points3D, img1idx, img2idx)

                    else: 
                        print(f'Skipping {prev_name} and {name} due to no inlier matches.')

                else: 
                    print(f'Skipping {prev_name} and {name} due to no matches.')
    ...
    def compute_reprojection_error(self, name): 
        ...
        # Select only keypoints that have a corresponding 3D point
        valid_indices = np.where(ref >= 0)[0]
        if len(valid_indices) == 0:
            print(f"No valid 3D references for image {name}.")
            return -1

        # Get corresponding 3D points
        points_3D = self.point_cloud[ref[valid_indices].astype(int)]
        
        # Project 3D points back to 2D
        projected_2D, _ = cv2.projectPoints(points_3D, R, t, self.K, distCoeffs=None)
        projected_2D = projected_2D.reshape(-1, 2)
        
        # Actual keypoints
        actual_2D = np.array([kp[idx].pt for idx in valid_indices])
        
        # Calculate reprojection error
        errors = np.sqrt(np.sum((projected_2D - actual_2D) ** 2, axis=1))
        err = np.mean(errors)
        ...
        if self.opts.plot_error:
            ...
            # Using draw_correspondences
            ax = draw_correspondences(image, actual_2D, projected_2D, ax)
            ax.set_title('reprojection error = {:.2f}'.format(err))
