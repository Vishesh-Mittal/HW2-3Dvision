...
def set_arguments(parser):
    """
    Set the arguments for the assignment.
    """
    ...
    parser.add_argument('--save_results',action='store', type=str, default=False, 
                        dest='save_results',help='[True|False] whether to save images with keypoints drawn on them (default: False)')  
...
def post_process_args(opts):
    ...
    Args:
        opts (object): The options object containing the command line arguments.
...
def feat_match(opts, data_files=[]):
    """
    Feature matching algorithm.

    Args:
        opts (object): The options.
        data_files (list): The data files.
    ...
    for i in range(len(data)): 
        for j in range(i+1, len(data)): 
            ...
            matches_ = serialize_matches(matches)
...
if __name__ == '__main__':
    ...
    # feature matching
    feat_match(opts, opts.data_files)
